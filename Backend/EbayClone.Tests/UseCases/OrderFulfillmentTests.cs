...
namespace EbayClone.Tests.UseCases
{
    public class OrderFulfillmentTests
    {
        ...
        [Fact]
        public async Task UpdateStatusToDelivered_ShouldCalculateCorrectFee()
        {
            // Arrange
            ...
            order.MarkAsShipped("Carrier", "TRK123");
            ...
            // Act
            await useCase.ExecuteAsync(shopId, orderId, new UpdateOrderStatusRequest { NewStatus = "DELIVERED" });

            // Assert
            Assert.Equal(5000, order.PlatformFee); // 5% of 100k
            Assert.Equal(95000, wallet.PendingBalance); // 95k added
